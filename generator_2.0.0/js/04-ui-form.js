...
$('system').addEventListener('input', () => {
    ...
    systemInputTimeout = setTimeout(() => {
        ...
        // Вторая кнопка «Сгенерировать» (в мастере)
        const quickGenerateBtn2 = $('quickGenerateBtn2');
        if (quickGenerateBtn2) {
            quickGenerateBtn2.addEventListener('click', (e) => {
                createRipple(e, quickGenerateBtn2);
                syncQuickFieldsToMain();
                // Скопировать логику из quickGenerateBtn
                const origBtn = $('quickGenerateBtn');
        ...
        function updateProgressBar(currentStep) {
            ...
            progressSteps.forEach((step, i) => {
                ...
                } else if (stepI < stepNum || (stepI < stepNum && stepsCompleted[i])) {
        ...
        if (quickHomeBtn) {
            quickHomeBtn.addEventListener('click', () => {
                if (quickStep1) quickStep1.classList.remove('hidden');
                if (quickStep2) quickStep2.classList.add('hidden');
                if (quickStep3) quickStep3.classList.add('hidden');
                if (quickStep4) quickStep4.classList.add('hidden');
                if (quickStep5) quickStep5.classList.add('hidden');
        ...
        if (quickNextBtn2) {
            quickNextBtn2.addEventListener('click', () => {
                ...
                quickStep3.classList.remove('hidden');
                updateHomeBtnVisibility();
                updateProgressBar(3);
        ...
        if (quickBackBtn3) {
            quickBackBtn3.addEventListener('click', () => {
                quickStep3.classList.add('hidden');
                quickStep2.classList.remove('hidden');
                updateHomeBtnVisibility();
                updateProgressBar(2);
            });
        ...
        if (quickBackBtn4) {
            quickBackBtn4.addEventListener('click', () => {
                quickStep4.classList.add('hidden');
                quickStep3.classList.remove('hidden');
                updateHomeBtnVisibility();
                updateProgressBar(3);
            });
        ...
        if (quickNextBtn3) {
            quickNextBtn3.addEventListener('click', () => {
                quickStep3.classList.add('hidden');
                quickStep4.classList.remove('hidden');
                updateHomeBtnVisibility();
                updateProgressBar(4);
            });
        }

        // Кнопки влияния (У кого?) — мультивыбор с комбинациями
        const impactGrid = $('impactGrid');
        if (impactGrid) {
        ...
        if (quickNextBtn4) {
            quickNextBtn4.addEventListener('click', () => {
                quickStep4.classList.add('hidden');
                quickStep5.classList.remove('hidden');
                updateHomeBtnVisibility();
                updateProgressBar(5);
        ...
        if (quickBackBtn5) {
            quickBackBtn5.addEventListener('click', () => {
                quickStep5.classList.add('hidden');
                quickStep4.classList.remove('hidden');
                updateHomeBtnVisibility();
                updateProgressBar(4);
            });
        ...
        // Кастомный dropdown для «Что сломалось?»
        ...
        progressSteps.forEach((step) => {
            ...
            step.addEventListener('click', () => {
                ...
                // Переключить видимость шагов
                [quickStep1, quickStep2, quickStep3, quickStep4, quickStep5].forEach((s, i) => {
                    if (s) s.classList.toggle('hidden', i + 1 !== targetStep);
                });
                updateHomeBtnVisibility();
