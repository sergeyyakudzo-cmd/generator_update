                        ...
                        if (data.updateAvailable) {
                            ...
                            if (confirmUpdate) {
                                ...
                                if (updateResult.success) {
                                    ...
                                    setTimeout(() => {
                                        alert('Обновление применено. Перезапустите генератор вручную.');
                                    }, 500);
                                } else {
                                    showStatus('Ошибка обновления: ' + updateResult.error, 'error');
                                }
                            }
                        } else if (data.error) {
                            showStatus('Проверка обновлений недоступна: ' + data.error, 'error');
                        } else {
                            showStatus('У вас последняя версия!', 'success');
                        }
                    } catch (err) {
                        console.error('[Update] Error:', err);
                        showStatus('Ошибка проверки обновлений: ' + err.message, 'error');
                    } finally {
