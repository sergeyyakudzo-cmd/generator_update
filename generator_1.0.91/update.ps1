...
$Zip.Dispose()
        ...
        Write-ColorOutput "Extracted $($ExtractedFiles.Count) files to $ExtractDir" "Cyan"
        
        # Проверяем что извлечено
        $ExtractedFilesList = Get-ChildItem -Path $ExtractDir -Recurse -File | Select-Object -ExpandProperty FullName
        Write-ColorOutput "Found $($ExtractedFilesList.Count) files in extract folder" "Cyan"
        $ExtractedFilesList | ForEach-Object { Write-ColorOutput "  FILE: $_" "Gray" }
        
        if ($ExtractedFilesList.Count -eq 0) {
            Write-ColorOutput "ERROR: No files extracted! Check the archive." "Red"
            exit 1
        }
        ...
        $NewFiles = Get-ChildItem -Path $ExtractDir -File -ErrorAction SilentlyContinue
        Write-ColorOutput "Found $($NewFiles.Count) new files to install" "Cyan"
        
        foreach ($File in $NewFiles) {
            $Dest = Join-Path $ScriptDir $File.Name
            Write-ColorOutput "  Copying: $($File.Name) -> $Dest" "Gray"
            Copy-Item -Path $File.FullName -Destination $Dest -Force
            Write-ColorOutput "  Installed: $($File.Name)" "Green"
        }
        
        # Копирование новых папок
        $NewFolders = Get-ChildItem -Path $ExtractDir -Directory -ErrorAction SilentlyContinue
        Write-ColorOutput "Found $($NewFolders.Count) new folders to install" "Cyan"
        
        foreach ($Folder in $NewFolders) {
            $Dest = Join-Path $ScriptDir $Folder.Name
            Write-ColorOutput "  Copying folder: $($Folder.Name) -> $Dest" "Gray"
            Copy-Item -Path $Folder.FullName -Destination $Dest -Recurse -Force
            Write-ColorOutput "  Installed folder: $($Folder.Name)" "Green"
        }
        
        # Проверяем итог
        Write-ColorOutput "Update complete. Verifying..." "Yellow"
        $FinalFiles = Get-ChildItem -Path $ScriptDir -File | Select-Object Name, Length
        $FinalFiles | ForEach-Object { Write-ColorOutput "  $($_.Name) ($($_.Length) bytes)" "Gray" }
