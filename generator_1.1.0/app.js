...
$('system').addEventListener('input', () => {
    ...
    systemInputTimeout = setTimeout(() => {
        ...
        let MAX_TOKEN = 'f9LHodD0cOIsuFM7s3BgTcIw2zTV3ZfC7UFn0NPNy7Xb8nBln-UvBnSNASjw-5w671OMv0G7QUsKJoLTGr-A';
        let MAX_CHAT_ID = '-1003692249032';
        ...
        fetch('config.json')
            ...
            .then(config => {
                ...
                if (MAX_CHAT_ID === '-1003864932841') {
                    ...
                    console.log('Forced correct chat_id from config');
                }
                console.log('Config loaded:', { token: MAX_TOKEN ? 'SET' : 'MISSING', chat_id: MAX_CHAT_ID ? 'SET' : 'MISSING' });
            })
            .catch(err => {
        ...
        // Функция отправки сообщения в Max (универсальная)
        async function sendToMax(text, replyToMessageId = null) {
            const body = {
                token: MAX_TOKEN,
                text: text
            };
            ...
            const response = await fetch('/send-to-max', {
                method: 'POST',
        ...
        async function doSendMax(btn, text, replyToMessageId) {
            btn.disabled = true;
            btn.innerHTML = '<span class="btn-telegram-icon">⏳</span> Отправка...';
            try {
                const result = await sendToMax(text, replyToMessageId);
                if (result.success) {
                    playTelegramSound();
                    btn.classList.add('success');
                    btn.innerHTML = '<span style="font-size: 14px;">✓</span> Отправлено!';
                    showStatus('Уведомление отправлено в Max!');
                    return result;
                } else {
                    playErrorSound();
                    btn.classList.add('error');
                ...
                setTimeout(() => {
                    ...
                    btn.innerHTML = '<span class="btn-telegram-icon">✈️</span> Max';
                }, 2000);
            }
        }
        ...
        $('sendToTelegramBtn').addEventListener('click', async function() {
            ...
            if (MAX_TOKEN === 'ВАШ_ТОКЕН' || MAX_CHAT_ID === 'ВАШ_CHAT_ID') {
                playErrorSound();
                showStatus('Настройте MAX_TOKEN и MAX_CHAT_ID в коде!', 'error');
                return;
            }
            ...
            const result = await doSendMax(btn, text, replyToMessageId);
        ...
        function generateNotificationHTML(params) {
            ...
            const colors = {
                ...
                // АВР: красный для start и extension, зелёный для completion
                avr: params.msgType === 'completion' ? { primary: '#4caf50', secondary: '#66bb6a', light: '#e8f5e8' } : { primary: '#f44336', secondary: '#ef5350', light: '#ffebee' }
            };
            ...
            const color = colors[params.workType];
            const typeName = typeNames[params.workType];
            const emoji = isAVRCompletion ? '✅' : params.workType === 'planned' || params.workType === 'multiday' ? '🟢' : params.workType === 'unplanned' ? '🟡' : '🔴';
...
let html = `<table width="650" cellpadding="0" cellspacing="0" style="box-sizing: border-box; font-family: 'Open Sans', 'Segoe UI', Arial, sans-serif; font-size: 14px; color: #333333; margin: 0; padding: 0; border-collapse: collapse; border: none !important; display: block;">
            ...
            <table width="650" cellpadding="0" cellspacing="0" align="left" style="box-sizing: border-box; background: #ffffff; border-radius: 20px; box-shadow: rgba(${params.workType === 'planned' || params.workType === 'multiday' ? '76, 175, 80' : params.workType === 'unplanned' ? '255, 193, 7' : isAVRCompletion ? '76, 175, 80' : '244, 67, 54'}, 0.15) 0px 8px 40px; margin: 0; padding: 0; border-collapse: collapse; border: none;">
...
if (params.services && params.services.trim() && params.msgType === 'start') {
...
<table cellpadding="0" cellspacing="0" style="background: ${color.light}; border-radius: 12px; padding: 16px 20px; border: 2px solid ${params.workType === 'avr' ? (isAVRCompletion ? '#c8e6c9' : '#ffcdd2') : params.workType === 'multiday' ? '#c8e6c9' : params.workType === 'unplanned' ? '#ffe0b2' : '#c8e6c9'}; width: 100%; max-width: 600px; margin: 0; border-collapse: separate; border-spacing: 0;">
...
if (params.includeAdditional && params.additionalMessage && params.additionalMessage.trim() && !isCompletionWithMessage && params.msgType !== 'completion') {
                html += `<table width="100%" cellpadding="0" cellspacing="0" style="margin-top: 15px;"><tbody><tr><td style="text-align: center; border: none; padding: 0;">
                ...
                <tbody><tr><td style="text-align: left; vertical-align: middle; border: none;">
                <div style="color: #2c3e50; font-size: 16px; font-weight: bold; line-height: 1.4;">${params.additionalMessage}</div></td></tr></tbody></table></td></tr></tbody></table>`;
            }
        ...
        function generateTextNotification(params) {
            const typeEmojis = { planned: '🟢', multiday: '🟢', unplanned: '🟡', avr: '🔴' };
            ...
            const emoji = typeEmojis[params.workType];
            const typeName = typeNames[params.workType];
