        document.addEventListener('DOMContentLoaded', () => {
            ...
            const generateBtn = $('quickGenerateBtn');
            if (generateBtn) {
        ...
        initUpdateButton();
            ...
            function showNotification(title, text) {
                ...
                if (!('Notification' in window)) {
                    console.log('Браузер не поддерживает уведомления, используем toast');
                    showNativeToast(title, text);
                    return;
                }
                ...
                if (Notification.permission === 'granted') {
                    ...
                    try {
                        ...
                        showNativeToast(title, text, 'error');
                    }
                    ...
                    Notification.requestPermission().then(permission => {
                        ...
                        if (permission === 'granted') {
                            ...
                            console.log('Разрешение отклонено, используем toast');
                            showNativeToast(title, text, 'error');
                        }
                    });
                } else {
                    console.log('Разрешение запрещено, используем toast');
                    showNativeToast(title, text, 'error');
                }
            }
            ...
            // Показать fallback-уведомление в правом нижнем углу
            function showNativeToast(title, text, type = 'success') {
                const toast = $('nativeToast');
                const icon = $('nativeToastIcon');
                const titleEl = $('nativeToastTitle');
                const textEl = $('nativeToastText');
            ...
            // Меню (три точки) — открытие/закрытие
            ...
            // Пункты меню
            $('menuTestNotification')?.addEventListener('click', function() {
                menu.style.display = 'none';
                $('testNotificationBtn')?.click();
            });

            $('menuCheckUpdate')?.addEventListener('click', function() {
                menu.style.display = 'none';
                $('updateBtn')?.click();
            });

            $('menuExportBackup')?.addEventListener('click', function() {
                menu.style.display = 'none';
                $('exportBtn')?.click();
            });

            $('menuImportBackup')?.addEventListener('click', function() {
                menu.style.display = 'none';
                $('importBtn')?.click();
            });
